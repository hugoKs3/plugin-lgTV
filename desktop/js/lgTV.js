...
$(function() {
    $("#table_cmd tbody").delegate(".listCmdlgTV", 'click', function(event) {
        $('.description').hide();
        $('.version').hide();
        $('.required').hide();
        $('.description.' + $('#sel_addPreConfigCmdlgTV').value()).show();
        $('.version.' + $('#sel_addPreConfigCmdlgTV').value()).show();
        $('.required.' + $('#sel_addPreConfigCmdlgTV').value()).show();
        $('#md_addPreConfigCmdlgTV').modal('show');
        $('#bt_addPreConfigCmdlgTVSave').undelegate().unbind();
        var tr = $(this).closest('tr');
        $("#div_mainContainer").delegate("#bt_addPreConfigCmdlgTVSave", 'click', function(event) {
            var cmd_data = json_decode($('.json_cmd.' + $('#sel_addPreConfigCmdlgTV').value()).value());
            tr.setValues(cmd_data, '.cmdAttr');
            $('#md_addPreConfigCmdlgTV').modal('hide');
        });
    });

    $("#sel_addPreConfigCmdlgTV").on('change', function(event) {
        $('.description').hide();
        $('.version').hide();
        $('.required').hide();
...
function addCmdToTable(_cmd) {
	...
	tr += '<a class="btn btn-default listCmdlgTV form-control input-sm" style="margin-top : 5px;"><i class="fa fa-list-alt cursor"></i> {{Ajouter une commande prédéfinie}}</a>';
    tr += '</td>';
    tr += '<td>';
